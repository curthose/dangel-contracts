...
BOOSTER_TOKEN_ID="token.dangelfund.near"

ORACLE_ID="priceoracle.near"

CONTRACT_ID="v1.dangelfund.near"

echo -e "$LG>>>>>>>>>>>>>>$TC Deploying contract account: $CONTRACT_ID $LG<<<<<<<<<<<<<<$NC"
...
echo -e "$LG>>>>>>>>>>>>>>$TC ADDING ASSET: $CONTRACT_ID $LG<<<<<<<<<<<<<<$NC"
...
echo -e "$LG>>>>>>>>>>>>>>$TC ADDING FARM  $CONTRACT_ID $LG<<<<<<<<<<<<<<$NC"
...
echo -e "export NEAR_ENV=mainnet"
