...
BOOSTER_TOKEN_ID="token.1678449335.baraka.testnet"
...
near create-account $CONTRACT_ID --masterAccount=$MAIN --initialBalance=20
near deploy $CONTRACT_ID target/wasm32-unknown-unknown/release/contract.wasm new '{"config": {
  ...
  "minimum_staking_duration_sec": 15,
...
near call $CONTRACT_ID --accountId=$OWNER_ID --gas=$GAS --amount=$ONE_YOCTO execute '{
  "actions": [
    {
      "Withdraw": {
        "token_id": "'$BOOSTER_TOKEN_ID'",
